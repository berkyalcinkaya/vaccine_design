...
'''CODE HAS BEEN MODIFIED BY BERK YALCINKAYA TO PRODUCE ENTROPY DATA IN CSV FORMAT'''
import argparse
import numpy as np
...
if __name__ == '__main__':
    ...
    ################################################################################
    ################################################################################
    ## CODE ADDED BY BERK YALCINKAYA TO THIS PROGRAM, THIS ADDITION WRITES ENTROPY DATA TO A FILE
    ## ENTROPY.CSV
    # write sequence entropies to csv
    seq_entropies_output = [round(e,4) for e in seq_entropies]
    pos = np.arange(1, len(seq_entropies) + 1).tolist()
    out = zip(pos,list(seq_entropies_output))
    with open("entropy.csv", "a") as file:
        csv_out = csv.writer(file)
        csv_out.writerow(["pos", "entropy"])
        for p, ent in out:
            csv_out.writerow([p,ent])
    ################################################################################
    ################################################################################
    ################################################################################
